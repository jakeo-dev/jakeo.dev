...
export default function Header() {
  ...
  // set local part of email to a new random local part from localParts array
  function newRandomEmail() {
    const newLocalPart = localParts.filter((str) => str !== currentLocalPart)[
      Math.floor(
        Math.random() *
          (localParts.length - (localParts.includes(currentLocalPart) ? 1 : 0)),
      )
    ];
